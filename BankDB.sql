...
CREATE TABLE Loan (
  ...
  InterestRate DECIMAL(5,2), -- percent
  LoanStatus VARCHAR(20), -- e.g. 'Open', 'Closed', 'Default'
  StartDate DATE,
  EndDate DATE,
  FOREIGN KEY (CustomerID) REFERENCES Customer(CustomerID),
  FOREIGN KEY (BranchID) REFERENCES Branch(BranchID)
...
CREATE TRIGGER trg_before_insert_tx BEFORE INSERT ON TransactionTable
FOR EACH ROW
BEGIN
  IF NEW.TransactionType = 'Debit' THEN
    ...
    IF curBal < NEW.Amount THEN
      SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Insufficient funds - transaction blocked';
    END IF;
  END IF;
END;
//
...
CREATE TRIGGER trg_after_insert_tx AFTER INSERT ON TransactionTable
FOR EACH ROW
BEGIN
...
BEGIN
  ...
  ELSE
    ...
    INSERT INTO TransactionTable(AccountID,TransactionType,Amount,Description) VALUES (from_acc,'Debit',amt,CONCAT('Transfer to ',to_acc));
    INSERT INTO TransactionTable(AccountID,TransactionType,Amount,Description) VALUES (to_acc,'Credit',amt,CONCAT('Transfer from ',from_acc));
    COMMIT;
  END IF;
END;
//
...
SELECT c.CustomerID, CONCAT(c.FirstName,' ',c.LastName) AS CustomerName, a.AccountID, t.TransactionID, t.TransactionDate, t.TransactionType, t.Amount, t.Description
FROM Customer c
...
BEGIN
  SELECT a.AccountID, t.TransactionID, t.TransactionDate, t.TransactionType, t.Amount, t.Description
  FROM Account a
  JOIN TransactionTable t ON a.AccountID = t.AccountID
  WHERE a.CustomerID = cust_id AND t.TransactionDate BETWEEN from_dt AND to_dt
...
GROUP BY AccountID
HAVING TotalDebits > 100000; -- threshold example
SELECT b.BranchID, b.BranchName, SUM(t.Amount) AS TotalDeposits
FROM Branch b
JOIN Account a ON b.BranchID = a.BranchID
JOIN TransactionTable t ON a.AccountID = t.AccountID
WHERE t.TransactionType='Credit' AND t.TransactionDate >= DATE_SUB(CURDATE(), INTERVAL 3 MONTH)
GROUP BY b.BranchID
ORDER BY TotalDeposits DESC;
SELECT AccountID, MonthlyDeposits,
       RANK() OVER (ORDER BY MonthlyDeposits DESC) AS rank
FROM (
  SELECT AccountID, SUM(CASE WHEN TransactionType='Credit' THEN Amount ELSE 0 END) AS MonthlyDeposits
  FROM TransactionTable
  WHERE TransactionDate >= DATE_FORMAT(CURDATE(), '%Y-%m-01')
  GROUP BY AccountID
) t;
-- rule: total balance > 1,00,000 and no large debit in last month
SELECT c.CustomerID, CONCAT(c.FirstName,' ',c.LastName) AS Name, SUM(a.Balance) AS TotalBalance
